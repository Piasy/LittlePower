...
派发一个 fresh subagent，使用下面的 prompt：
  ...
  prompt: |
    ...
    - Worktree 状态：controller 直接把当前 controller worktree、BASE..HEAD 集成结果、完整测试和 smoke check 交给你执行完整收尾验证。

    ### 模式 B：Focused re-review
    - Review 类型：focused re-review
    - Controller worktree：[WORKTREE_PATH]
    - 上一轮收尾 verdict：[PRIOR_COMPLETION_VERDICT]
    - 收尾 fixer 报告：[FINAL_FIXER_REPORT]
    - Git index 状态：[GIT_INDEX_PROTOCOL: controller 已在上一轮 verdict 后固定已审 baseline；staged changes 是上一轮已审基线；unstaged changes 是本轮收尾修复]
