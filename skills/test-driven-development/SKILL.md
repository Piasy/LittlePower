...
编写或修改任何测试时，都要阅读 [writing-good-tests.md](writing-good-tests.md)，使用其中的测试质量门禁：
- 先说清这个测试要捕获哪个生产代码 break。
- 期望值必须独立推导，不能复用被测代码的逻辑。
- 测自己的边界契约和真实行为，不测 mock、源码文本或框架内部机制。
- 完成前做 mutation check，确认现实中的错误改动会被测试抓住。

这份参考适用于产品行为 TDD，也适用于 tests-only 的测试维护。读取它不代表 tests-only 任务要启动 RED-GREEN-REFACTOR。
...
| “我之后再测” | 对产品行为变更，生产代码之后写的测试一上来就通过，而直接通过证明不了它能抓住缺失行为：它可能测错东西、测实现而非行为，或漏掉你忘记的边界情况。你从未看着它抓住 bug；测试先写会强迫这次证明发生。 |
| “事后补测试也一样，重要的是精神不是仪式” | 对产品行为变更，事后补测回答“这会做什么？”，测试先写回答“这应该做什么？”。测试会被已有实现反向塑形，只验证你记得的情况，而不是在实现前发现遗漏；你得到了覆盖率，却没有证明测试真的能抓问题。 |
| “我已经手工测过了” | 手工测试是临时性的：没有覆盖记录，代码变化后不能稳定重跑，压力下也容易漏场景。“我试过一次是好的”不等于覆盖全面；自动化测试每次都会以同样方式运行。 |
| “撤回 X 小时的生产代码太浪费” | 这是沉没成本，时间已经花出去了。真正的选择是按 TDD 重写并获得高信心，或把测试事后钉在实现上并接受低信心和潜在 bug；把自己都不信任的代码留下来才是浪费。 |
| “先留着生产实现当参考，再写测试” | 你会忍不住按它改。那仍然是事后补测。撤回就是撤回。 |
| “我得先探索一下” | 可以。探索完就丢掉实验性生产代码，然后从 TDD 重新开始。 |
| “这个太难测试了” | 那通常说明设计不清晰。听测试的反馈。难测 = 难用。 |
| “TDD 太教条了，会拖慢我” | TDD 才是务实路径：它在提交前抓 bug、防止回归、记录行为，并让重构可控。所谓“务实”的捷径通常把问题推到线上调试，最终更慢。 |
| “手工测更快” | 手工测证明不了边界情况，而且每改一次都得重测。 |
| “现有生产代码本来就没测试” | 现在正好轮到你补上。 |

这些论证只适用于生产代码行为变更。给已有行为补覆盖、迁移测试、修 flaky 或维护 fixture 时，测试一开始通过是正常的；不要为了 tests-only 工作制造假 RED。
