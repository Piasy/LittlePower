...
派发一个 fresh subagent，使用下面的 prompt：
  ...
  prompt: |
    ...
    **目标项目仓库：** [REPO_PATH]
    ...
    | 共享测试基线 | 是否存在 `Shared Verification Baseline` 子段，抽出 feature slice 通用的主路径 boilerplate、默认断言层级、防 mock 逃逸禁令、调试产物落盘约定 |
    ...
    | 拆分追踪 | 如果拆成多个 child spec，父 spec 是否用 `Split Specs` 追踪已写出 child spec，是否用 `Candidate Future Split Specs` 列未来候选，且 child spec 文件名是否继承父 spec 前缀并使用 `child-x`；是否没有把 child spec 称为 slice |
    | 链接与路径 | parent/child spec 的互相引用是否使用 markdown 链接，且链接目标使用相对当前文档位置可解析的相对路径，而不是纯文本路径 |
    | 共享约束继承 | 如果拆成多个 child spec，共享 `Global Constraints` 是否只保留在父 spec；child spec 是否通过相对链接引用父 spec 的共享约束，并只写本 child spec 的增量约束 |
    ...
    **只指出会在实现期间造成真实问题的事项。**
    ...
    如果需求被拆成多个 child spec，请同时读取 `skills/brainstorming/split-spec-conventions.md`，并按其中的术语规则、拆分追踪、命名规则、链接规则和 `Global Constraints` 继承规则审查。
    不要批准任何违反该共享约定的拆分方案。

    ## 验收章节结构标准

    `Acceptance Criteria` 是 feature slice 中承载验收行为契约和验证手段的唯一章节，需同时覆盖二者。
    章节应包含一个 `Shared Verification Baseline` 子段，抽出 feature slice 通用的主路径、默认断言层级、防 mock 逃逸禁令、调试产物落盘约定，避免每条 AC 各自重述。
    ...
    **Scope and Split Assessment:**
    - [说明 spec 是否足够小；是否命中拆分信号；如果超过约 500 行，说明是否应拆分；如果已拆分，说明 `Split Specs`、`Candidate Future Split Specs`、checkbox 使用、parent/child markdown 相对链接、child spec 命名以及 child spec / feature slice 术语是否正确；同时说明共享 `Global Constraints` 是否在父 spec 收敛、child spec 是否只做引用和增量补充]
